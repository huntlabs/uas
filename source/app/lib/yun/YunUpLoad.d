...
class YunUpLoad
{
	this()
	{
		...
		_appid = con.filecloud.appid.value;
		_baseurl = con.filecloud.host.value;
		_uploadtoken = buildToken(con.filecloud.public_key.value , con.filecloud.secret_key.value);
	}
	...
	string doUpload(const byte[] data , string filename = "test")
	{
		...
		if(json["hash"].str != ""){
			logInfo(json);
			json["url"] = con.filecloud.read.value ~ json["hash"].str ~ ".png";
		}else{
			logInfo(json);
		}
...
protected:
	...
	static string buildToken(string publickey , string signkey)
	{
		...
		string str = "{\"deadline\":" ~ to!string((cast(int)deadline + 60)) ~ "}";
